...
exports.createChannel = async (req, res) => {
    try {
        const { platform, channel_name, channel_id, access_token, channel_secret, webhook_url } = req.body;
        ...
        const insertId = await Channel.create({ platform, channel_name, channel_id, access_token, channel_secret, webhook_url, created_by });
...
exports.updateChannel = async (req, res) => {
    try {
        const { channel_name, channel_id, access_token, channel_secret, webhook_url } = req.body;
        if (!channel_name) {
            return res.status(400).json({ status: 'error', message: 'กรุณาระบุ channel_name' });
        }
        await Channel.update(req.params.id, { channel_name, channel_id, access_token, channel_secret, webhook_url });
