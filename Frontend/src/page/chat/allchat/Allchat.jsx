  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    parts.push(
      <img
        ...
        src={`https://stickershop.line-scdn.net/sticonshop/v1/sticon/${productId}/android/${emojiId}.png`}
        alt="emoji"
        style={{
          width: size,
          height: size,
...
const MiniChatPanel = ({
  ...
  const handleBlur = () => {
    if (focusedNodeRef) {
      setTimeout(() => {
        if (
          focusedNodeRef.current === customer.id &&
    ...
    <div className="mini-chat-panel" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="mini-chat-messages" ref={messagesContainerRef}>
        {chatMessages.map((msg) => (
          ...
          >
            ...
            <div
              className="mini-msg-content-wrapper position-relative"
              style={{
                display: "flex",
            ...
            >
                ...
                <div
                  ...
                  onClick={() => {
                    const target = document.getElementById(
                      `mini-msg-${customer.id}-${msg.reply_to_id}`,
                    );
                    if (target) {
                ...
                >
                  ...
                  <span className="ms-1" style={{ opacity: 0.8 }}>
                    {msg.reply_preview_text ||
                      (msg.reply_preview_image ? "📷 รูปภาพ" : "ข้อความ")}
                  </span>
                </div>
              )}
              ...
              {msg.message_type === "sticker" ? (
                ...
                >
                  {(() => {
                    try {
                      ...
                      return <span>Invalid carousel</span>;
                    }
                  })()}
                </div>
              ) : msg.image ? (
                ...
                <span style={{ lineHeight: 1.2 }}>
                  {renderTextWithLineEmoji(msg.text, 36)}
                </span>
      ...
      {showTemplatePicker && (
        ...
        >
          <TemplatePicker
            onSelectText={(text) => {
              onSend(customer.id, text);
              setShowTemplatePicker(false);
            }}
            onSelectImage={(url) => {
              /* image templates not common */ setShowTemplatePicker(false);
            }}
            onSelectCarousel={(cards) => {
              if (onSendCarousel) onSendCarousel(customer.id, cards);
              setShowTemplatePicker(false);
            }}
                ...
                setReplyTo({
                  ...
                  preview_text:
                    contextMenu.msg.message_type === "text"
                      ...
                      : null,
                  preview_image:
                    contextMenu.msg.message_type === "image"
                      ? contextMenu.msg.image
                      : null,
                });
                setContextMenu(null);
                inputRef.current?.focus();
      ...
      {pastedImage && (
        ...
        >
          ...
          <div className="image-picker-grid" style={{ padding: "8px 12px" }}>
            <div
              className="image-picker-thumb selected"
              style={{ width: "70px", height: "70px", cursor: "default" }}
            >
              <img src={pastedImage.previewUrl} alt="preview" />
              <div
                className="image-picker-check"
                style={{
                  width: "18px",
                  height: "18px",
                  top: "3px",
      ...
      {replyTo && (
        <div
          className="reply-preview-bar d-flex align-items-center justify-content-between p-2"
          style={{
            backgroundColor: "var(--bg-surface)",
            borderTop: "1px solid var(--border-medium)",
          }}
          ...
          >
            ...
            <span>
              {replyTo.preview_text ||
                (replyTo.preview_image ? "📷 รูปภาพ" : "ข้อความ")}
            </span>
          </div>
          <button
            type="button"
          ...
          onKeyDown={(e) => {
            if (e.key === "Tab") {
              ...
              const inputs = Array.from(
                document.querySelectorAll('input[id^="chat-input-"]'),
              );
              ...
              const currentIndex = inputs.findIndex(
                (input) => input.id === `chat-input-${customer.id}`,
              );
              if (currentIndex !== -1) {
                ...
                const targetCustomerId = inputs[nextIndex].id.replace(
                  "chat-input-",
                  "",
                );
                const card = document.getElementById(
                  `allchat-card-${targetCustomerId}`,
                );
                if (card) {
...
const AllChat = () => {
  ...
  const getLastMsgTime = useCallback(
    (customerId) => {
  ...
  const getSortMsgTime = useCallback(
    (customerId) => {
      const msgs = messages[customerId];
      if (!msgs || msgs.length === 0) return 0;
  ...
  const filteredCustomers = useMemo(() => {
    return customers
      .filter((c) => {
        // 1. Status tab filter
        if (activeFilter === "not_started" && c.status !== STATUS.NOT_STARTED)
          return false;
        if (activeFilter === "in_progress" && c.status !== STATUS.IN_PROGRESS)
          return false;
        ...
        if (filterAssignee !== "all") {
          ...
          if (
            ...
            c.assigned_to !== Number(filterAssignee)
          )
            return false;
        }
        ...
        if (
          filterChannelId !== "all" &&
          c.channel_id !== Number(filterChannelId)
        )
          return false;
  ...
  const statusCounts = {
    ...
    not_started: customers.filter((c) => c.status === STATUS.NOT_STARTED)
      .length,
    in_progress: customers.filter((c) => c.status === STATUS.IN_PROGRESS)
      .length,
  ...
  const handleToggleContextTag = useCallback(
    async (customerId, tagId) => {
      const token = sessionStorage.getItem("token");
      try {
  ...
  const renderUserCard = (customer, isActive) => {
    ...
    const assignedMember = members.find(
      (m) => m.emp_id === customer.assigned_to,
    );
        ...
        <div className="user-card-main">
          ...
          <div className="user-card-info">
            ...
            <div className="user-card-bottom">
              ...
              {lastTime && (
                <span className="user-card-time">{formatTime(lastTime)}</span>
              )}
            </div>
            {customer.platform && (
              <div
              ...
              >
                {customer.platform === "line" ? (
                  <svg
                    ...
                    fill="#06C755"
                  >
                    <path d="M12 2C6.48 2 2 5.88 2 10.54c0 4.24 3.76 7.78 8.84 8.44.34.07.81.22.93.52.1.27.07.68.03.95l-.15.91c-.05.27-.22 1.06.93.58s6.19-3.65 8.44-6.25C22.97 13.42 22 12.06 22 10.54 22 5.88 17.52 2 12 2z" />
                  </svg>
                ) : (
                  <i
                    className="bi bi-messenger"
                    style={{ color: "#0084FF", fontSize: "12px" }}
                  ></i>
                )}
                <span
                  style={{
                ...
                {assignedMember.image ? (
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "50%",
                      flexShrink: 0,
                      backgroundColor: getAvatarColor(assignedMember.username),
                      fontSize: "10px",
                      fontWeight: "600",
                    }}
                  >
        ...
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 w-100">
          ...
          <div className="d-flex align-items-center flex-wrap gap-2">
            ...
            >
              <Dropdown.Toggle
                as="div"
                className="nav-search shadow-none pointer"
              >
                <i
                  className={`bi bi-${filterPlatform === "line" ? "line" : filterPlatform === "facebook" ? "messenger" : "grid"} me-1`}
                ></i>
                {filterPlatform === "all"
                  ? "ทุกช่องทาง"
                  : filterPlatform === "line"
              ...
              <Dropdown.Menu
                className="p-2 border-0 shadow-sm rounded-3"
                style={{
                  backgroundColor: "var(--bg-main)",
                  border: "1px solid var(--border-light)",
                }}
              >
                <Dropdown.Item eventKey="all" className="rounded">
                  <span style={{ color: "var(--text-main)" }}>ทุกช่องทาง</span>
                </Dropdown.Item>
                <Dropdown.Item eventKey="line" className="rounded">
                  <span style={{ color: "var(--text-main)" }}>LINE OA</span>
                </Dropdown.Item>
                <Dropdown.Item eventKey="facebook" className="rounded">
                  <span style={{ color: "var(--text-main)" }}>Messenger</span>
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
            ...
            <Dropdown onSelect={(val) => setFilterChannelId(val)}>
              ...
              >
                ...
                {filterChannelId === "all"
                  ? "ทุกร้าน"
                  : allChannels.find((ch) => ch.id === Number(filterChannelId))
                      ?.channel_name || "เลือกร้าน"}
                <i
                  className="bi bi-chevron-down ms-1"
                  style={{ fontSize: "10px" }}
                ></i>
              ...
              >
                <Dropdown.Item eventKey="all" className="rounded">
                  <span style={{ color: "var(--text-main)" }}>ทุกร้าน</span>
                </Dropdown.Item>
                {allChannels
                  .filter(
                    (ch) =>
                      filterPlatform === "all" ||
                      ch.platform === filterPlatform,
                  )
                  .map((ch) => (
                    <Dropdown.Item
                      key={ch.id}
                      eventKey={ch.id.toString()}
                      className="rounded"
                    >
            ...
            <Dropdown>
              <Dropdown.Toggle as="div" className="nav-search shadow-none">
              ...
              >
                <Dropdown.Item
                  ...
                  className="rounded"
                >
                  <span style={{ color: "var(--text-main)" }}>ล่าสุด</span>
                </Dropdown.Item>
                <Dropdown.Item
                  onClick={() => setSortOrder("oldest")}
                  className="rounded"
                >
                  <span style={{ color: "var(--text-main)" }}>เก่าสุด</span>
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
            ...
            <Dropdown>
              <Dropdown.Toggle as="div" className="nav-search shadow-none">
                ...
                {filterAssignee === "all"
                  ...
                  : filterAssignee === "unassigned"
                    ? "ยังไม่ได้กำหนด"
                    : members.find((m) => m.emp_id === filterAssignee)
                        ?.username || filterAssignee}
                <i
                  className="bi bi-chevron-down ms-1"
                  style={{ fontSize: "10px" }}
                ></i>
              ...
              >
                <Dropdown.Item
                  ...
                  className="rounded"
                >
                  <span style={{ color: "var(--text-main)" }}>ทั้งหมด</span>
                </Dropdown.Item>
                <Dropdown.Item
                ...
                {members.map((m) => (
                  <Dropdown.Item
                    key={m.emp_id}
                    onClick={() => setFilterAssignee(m.emp_id)}
                    className="rounded"
                  >
                    <span style={{ color: "var(--text-main)" }}>
                      {m.username}
            ...
            <Dropdown>
              ...
              >
                {globalTags.length === 0 ? (
                  ...
                  >
                    ยังไม่มีแท็ก กรุณาเพิ่มในหน้าตั้งค่า
                  </div>
                ) : (
                  globalTags.map((tag) => (
                    <Form.Check
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            ...
            >
              <span>
                <i className="bi bi-circle-half me-2"></i>สถานะ
              </span>
              <i
            ...
            >
              {Object.entries(STATUS_STYLE).map(([status, style]) => (
                ...
                >
                  <span style={{ color: style.bg, fontWeight: 500 }}>
                    {status}
                  </span>
                </Dropdown.Item>
              ))}
          ...
          >
            <Dropdown.Toggle
              ...
              onClick={() =>
                setOpenSubMenu(openSubMenu === "assignee" ? null : "assignee")
              }
            >
              <span>
                <i className="bi bi-person-check me-2"></i>ผู้รับผิดชอบ
              </span>
              <i
                className="bi bi-chevron-right"
                style={{ fontSize: "10px" }}
            ...
            >
              ...
              <Dropdown.Divider
                style={{ borderColor: "var(--border-medium, #e5e7eb)" }}
              />
              {members.map((m) => (
                <Dropdown.Item
                  key={m.emp_id}
          ...
          <Dropdown drop="end" className="w-100" show={openSubMenu === "tag"}>
            <Dropdown.Toggle
              ...
              onClick={() =>
                setOpenSubMenu(openSubMenu === "tag" ? null : "tag")
              }
            >
              <span>
                <i className="bi bi-tags me-2"></i>แท็ก
              </span>
              <i
                className="bi bi-chevron-right"
                style={{ fontSize: "10px" }}
            ...
            >
              {globalTags.length === 0 ? (
                ...
                >
                  {globalTags.map((t) => {
                    const cTags =
                      customerTagsMap[contextMenu.customer.id] || [];
