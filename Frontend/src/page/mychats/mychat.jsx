...
// === Mini Chat Panel ===
const MiniChatPanel = ({ customer, chatMessages, onOpenFull, onClose, onSend, draftRefs, focusedNodeRef }) => {
...
const MyChat = ({ currentUser }) => {
    ...
    const filteredCustomers = useMemo(() => {
        return customers.filter((c) => {
            ...
            // 2. Tag Filter (cTagIds array must contain ALL selected tags)
            ...
            >
                ...
                <div className="user-card-footer">
                    ...
                    <div className="tag-group">
                        {(customerTagsMap[customer.id] || []).map((tag, i) => (
                            <span
                                key={tag.id || i}
                                className="tag-badge"
                                ...
                                title={tag.text}
                            >
                                {tag.text.length > 9 ? tag.text.substring(0, 8) + '…' : tag.text}
                            </span>
                        ))}
                    </div>
                </div>
                                    ...
                                    <Form.Check
                                        ...
                                        label={
                                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                                                <span style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: tag.color }}></span>
                                                {tag.text}
                                            </span>
                                        }
                                        className="mb-1"
