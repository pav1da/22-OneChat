    ...
    if (file) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // อัปเดต sessionStorage ให้ Sidebar แสดงรูปใหม่ทันที
          const savedUser = JSON.parse(
            sessionStorage.getItem("myAppUser") || "{}",
          );
          savedUser.image = data.imageUrl;
          sessionStorage.setItem("myAppUser", JSON.stringify(savedUser));

          // แจ้ง App.jsx ให้ re-read currentUser
          window.dispatchEvent(new Event("user-updated"));
        } else {
  ...
  return (
    ...
    ------------------------------------------ */}
        ...
        <Row className="align-items-center justify-content-between">
          ...
          <Col xs="auto">
            <Button
              variant="dark"
              style={btnDarkStyle}
              onClick={() => alert("ฟีเจอร์ 2FA จะเปิดให้ใช้งานเร็วๆ นี้")}
            >
              เปิดใช้งาน 2FA
            </Button>
          </Col>
      ...
      >
        ...
        <Modal.Body
          style={{ padding: "20px 40px 40px 40px", textAlign: "center" }}
        >
          <div
            style={{
              marginBottom: "24px",
          ...
          >
            เราจำเป็นต้องยืนยันที่อยู่อีเมลเก่าของคุณ
            <br />
            <span style={{ fontWeight: "bold" }}>{userData.email}</span>{" "}
            เพื่อเปลี่ยนแปลงอีเมล
          </div>
      ...
      >
        ...
        >
          <Button
            variant="light"
            onClick={handleCloseEmailReasonModal}
            style={modalBtnCancel}
          >
            ยกเลิก
          </Button>
